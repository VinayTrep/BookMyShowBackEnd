...
public record AuditoriumResponseDto(UUID id, String auditoriumName,int capacity,List<BMSShowResponseDto> shows,
    ...
    public static AuditoriumResponseDto fromAuditorium(Auditorium auditorium) {
        List<BMSShowResponseDto> shows = auditorium.getShows().stream().map(BMSShowResponseDto::fromShow).toList();
